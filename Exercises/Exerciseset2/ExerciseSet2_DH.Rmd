...
1. 6
<br>
2. 5y and 10y T-notes (0.93)
<br>
3. 1: Government bonds, 2: FX rates, 
<br>
4.a
...
Variance_Explained_Table_pre_crisis<-cbind(paste("component",1:ncol(pre_crisis_data),sep=" "),Variance_Explained_Table_results_pre_crisis$eig)
Variance_Explained_Table_pre_crisis<-as.data.frame(Variance_Explained_Table_pre_crisis)
colnames(Variance_Explained_Table_pre_crisis)<-c("Component","Eigenvalue", "Percentage_of_explained_variance", "Cumulative_percentage_of_explained_variance")
```

```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE, results='asis'}
show_data_pre_crisis = data.frame(Variance_Explained_Table_pre_crisis)
m2<-gvisTable(show_data_pre_crisis,options=list(width=1920, height=min(400,27*(nrow(show_data_pre_crisis)+1)),allowHTML=TRUE,page='disable'),formats=list(Eigenvalue="#.##",Percentage_of_explained_variance="#.##",Cumulative_percentage_of_explained_variance="#.##"))
print(m2,'chart')
```

```{r echo=TRUE, comment=NA, warning=FALSE, error=FALSE,message=FALSE,results='asis',tidy=TRUE}
corused1 = cor(pre_crisis_data[,apply(pre_crisis_data!=0,2,sum) > 10, drop=F])
Rotated_Results1<-principal(corused1, nfactors=20, rotate="varimax",score=TRUE)
Rotated_Factors1<-round(Rotated_Results1$loadings,2)
Rotated_Factors1<-as.data.frame(unclass(Rotated_Factors1))
colnames(Rotated_Factors1)<-paste("Component",1:ncol(Rotated_Factors1),sep=" ")

sorted_rows1 <- sort(Rotated_Factors1[,1], decreasing = TRUE, index.return = TRUE)$ix
Rotated_Factors1 <- Rotated_Factors1[sorted_rows,]
Rotated_Factors1[abs(Rotated_Factors1) < 0.3]<-NA
```

```{r echo=FALSE, comment=NA, warning=FALSE, error=FALSE,message=FALSE,results='asis'}
show_data_pre_crisis <- Rotated_Factors1 
show_data_pre_crisis<-cbind(rownames(show_data_pre_crisis),show_data_pre_crisis)
colnames(show_data_pre_crisis)<-c("Variables",colnames(Rotated_Factors1))
m2<-gvisTable(show_data_pre_crisis,options=list(showRowNumber=TRUE,width=1220, height=min(400,27*(nrow(show_data_pre_crisis)+1)),allowHTML=TRUE,page='disable'))
print(m2,'chart')
```
...
```{r echo=TRUE, comment=NA, warning=FALSE, error=FALSE,message=FALSE,results='asis',tidy=TRUE}
corused2 = cor(crisis_data[,apply(crisis_data!=0,2,sum) > 10, drop=F])
...
colnames(Rotated_Factors2)<-paste("Component",1:ncol(Rotated_Factors2),sep=" ")

sorted_rows2 <- sort(Rotated_Factors2[,1], decreasing = TRUE, index.return = TRUE)$ix
Rotated_Factors2 <- Rotated_Factors2[sorted_rows,]
Rotated_Factors2[abs(Rotated_Factors2) < 0.3]<-NA
```

```{r echo=FALSE, comment=NA, warning=FALSE, error=FALSE,message=FALSE,results='asis'}
show_data_crisis <- Rotated_Factors2 
show_data_crisis<-cbind(rownames(show_data_crisis),show_data_crisis)
colnames(show_data_crisis)<-c("Variables",colnames(Rotated_Factors2))
m3<-gvisTable(show_data_crisis,options=list(showRowNumber=TRUE,width=1220, height=min(400,27*(nrow(show_data_crisis)+1)),allowHTML=TRUE,page='disable'))
print(m3,'chart')
```

<br>
5. Correlation over all investment classes is higher during bear markets than bull markets. As well component 1 becomes more dominant in explaining variance (almost 30%) and th e top 4 components describing more than 50%.
<hr>
...
<br>
<br>
1. It is a trading strategy that based on the analysis of the moving average indicates whether to go long or short the market. Investment capital is 1 dollar. The singal indicates on which days you should buy or sell.
<br>
2.
```{r echo=TRUE, eval=TRUE, comment=NA, warning=FALSE,error=FALSE, message=FALSE, prompt=FALSE, tidy=TRUE}
signal_used1 = 0*futures_data # just initialize the trading signal to be 0
# Take many moving Average (MA) Signals and let them "vote" with their sign (+-1, e.g. long or short vote, for each signal)
MAfreq1<-seq(1,361,by=10)
for (iter in 1:length(MAfreq1))
  signal_used1 = signal_used1 + sign(apply(futures_data,2, function(r) ma(r,MAfreq1[iter])))
# Now make sure we invest $1 every day (so the sum of the absolute values of the weights is 1 every day)
signal_used1 = t(apply(signal_used1,1,function(r) {
  res = r  
  if ( sum(abs(r)) !=0 )
    res = r/sum(abs(r))
  res
}))
colnames(signal_used1) <- colnames(futures_data)
# Now create the returns of the strategy for each futures time series
strategy_by_future <- scrub(shift(signal_used1,2)*futures_data) # use the signal from 2 days ago
# finally, this is our futures trend following strategy
trading_strategy1 = apply(strategy_by_future,1,sum)
names(trading_strategy1) <- rownames(futures_data)
```


```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig.height=5,fig.width= 8, fig=TRUE}
pnl_plot(trading_strategy1)
```

```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE, results='asis'}
show_data = data.frame(cbind(rownames(pnl_matrix(trading_strategy1)), round(pnl_matrix(trading_strategy1),2)))
m2<-gvisTable(show_data,options=list(width=1220, height=min(400,27*(nrow(show_data)+1)),allowHTML=TRUE))
print(m2,'chart')
```
<hr>
