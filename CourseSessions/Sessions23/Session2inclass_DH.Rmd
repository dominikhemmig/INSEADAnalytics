...
<br>
<br>
1. Yes, as expected there is higher correlation between questions that cover similar topics. 
2. Correlations actualyl imply that different questions address a similar overarching topic and hence do only add slightly more value as these similar questions might be summarized in a general factor/question.
<hr>
...
<br>
<br>
1. The table shows how much variance is explained by each component, running from the highest Eigenvalue to the lowest. The plot is the visualization of the table. we can learn from these that the first five components actually explain the most variance and any component thereafter explains less than an individual question.
2. The main reason is that the most important components are shown on the left, where we usually start reading a chart. However, one could also have an increasing line, then the last components to the right would be the important ones.
3. Very few components explaining a maximum of variance.
<hr>
...
<br>
<br>
1.Yes, component #1 covers the topic of status, status symbol etc. Component #2 is about being active and in nature. #3 is about technology and quality. #4 is about durability and #5 is about cost sensitivity.
2. a) only consider factors with eigenvalue >1; b) 5 segments could make sense, as the types of customers are very distinctive c) This would have to be anserwed in combination with know-how of how much it costs to cater these 5 segments with different products. If the cost to develop and produce distinctive products for the segments is lower than the the potential benefits for the company, they should serve all 5. Otherwise it might make sense to group them and poentially sell the same product to two different segments, highlighting other features.
3. See answer 1   
4. I learn that data science can give us clear cut answers. However, these answers must be looked at and interpreted from a business perspective in order to validate and give context.
<hr>
<hr>
...
<br>
<br>
1. Looking at the dendogram, I would choose 3 segments
2. #1 social boater, non-enthusiastic; 2# future boat purchaser; #3 enthusiast with knowledge about boats
3. Smaller micro-segments are possible, however, it is questionable whether they are very distinctive and can be reasonably addressed individually.
4. The firm should target segment 3 based on their perception of quality, technology and status. Segment 3 tends to have in-depth knowledge and knows that price and quality do not only correlate up to a certain extent.
<hr>
<hr>
